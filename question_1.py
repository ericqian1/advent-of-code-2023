...
sum_val = 0
for code in input_vals.split('\n'):
	len_code = len(code)
	nums = []
	for i in range(len_code):
		if code[i] in valid_numbers:
			nums.append(code[i])
		else:
			for str_len, dict_len in number_dict.items():
				if len_code-i < str_len:
					break
				if code[i:i+str_len] in dict_len:
					nums.append(dict_len[code[i:i+str_len]])
	print(code, nums)
	sum_val += int(nums[0])*10 + int(nums[-1])
