input_txt = """F.F77.F7F7FFJJ..77FF.--L777.-7777--F7F7FJFFF7FF|.F77FF|-7-|-F7-7-FF|FJ-FFJ7-F--F|-L--7L|-7FFFF---7-FJ.7J.F-7J.F|7-LF7L7|.F-L|7...LF--LL7F-77
...
grid_txt = input_txt.split('\n')
grid_x, grid_y = len(grid_txt[0]), len(grid_txt)
sparse = {}


for x in range(grid_x):
	for y in range(grid_y):
		grid_val = grid_txt[y][x]
		if grid_val in transfers and grid_val!='S':
			sparse[(x,y)] = [(x+i,y+j) for i,j in transfers[grid_val]]
		if grid_val=='S':
			start = (x,y)
...
connected = [start]
next_connected = []
for (x,y), arr in sparse.items(): 
	if connected[0] in arr:
		connected.append((x,y))
		if start != arr[0]:
			next_connected.append(arr[0])
		else:
			next_connected.append(arr[1])


connected.pop(0)
loop_memo_1 = [start, connected[0]]
loop_memo_2 = [start, connected[1]]

ctr = 1
while(connected[0] != connected[1]):
	dummy = next_connected.copy()
	for i in range(2):
		arr = sparse[next_connected[i]]
		if connected[i] != arr[0]:
			next_connected[i] = arr[0]
		else:
			next_connected[i] = arr[1]
	ctr += 1
	connected = dummy
	loop_memo_1.append(connected[0])
	loop_memo_2.append(connected[1])

loop = loop_memo_1 + loop_memo_2[::-1]
doubled_loop = [(i*2,j*2) for (i,j) in loop]

expanded_loop = []

for i in range(1, len(doubled_loop)):
	c1 = doubled_loop[i-1]
	c2 = doubled_loop[i]
	expanded_loop.append(c1)
	expanded_loop.append(((c1[0]+c2[0])/2,(c1[1]+c2[1])/2 ))

expanded_loop.append(doubled_loop[-1])

import numpy as np 
import pandas as pd
from skimage.morphology import flood_fill

test = np.zeros((grid_x*2, grid_y*2))

for (i,j) in expanded_loop:
	test[int(i),int(j)] = 1

# fill matrix
filled = flood_fill(test, (0, 0), 1)
# revert matrix
filled = filled[::2, ::2]

print(grid_x * grid_y - filled.sum())
